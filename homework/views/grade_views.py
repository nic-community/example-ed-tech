...
from ..models import *
from ..repositories import HomeworkGradeRepository


class GradeViewSet(viewsets.ViewSet):
    
    def list(self, request):
        grades = HomeworkGradeRepository.get_all()
    ...
    def create(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            grade = HomeworkGradeRepository.filter(pk=serializer.data["id"])
            return Response(grade.values()[0], status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def retrieve(self, request, pk=None):
        queryset = HomeworkGradeRepository.get_all()
